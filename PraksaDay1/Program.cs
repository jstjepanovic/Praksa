...
namespace PraksaDay1
{
    ...
    class Program
    {
        interface IEquatable<T> // interface za funkciju određivanja jednakosti
        {
            bool Equals(T obj);
        }
        public abstract class Vehicle  // apstrakna klasa vehicle 
        {

            public int ProductionYear { get; set; }  // svako vozilo ima godinu proizvodnje i prijeđene kilometre
            public double TotalTravelLengthKm { get; set; }
            public abstract void Travel(double timeH, double avgSpeedKmH); // isto tako svako vozilo mora imati metodu travel

        }

        public class Car : Vehicle, IEquatable<Car> // Car nasljeđuje Vehicle i interface IEquatable
        {
            public string Color;
            private string Model { get; set; }
            public Car(int year, string model, string color)  // dva konstruktora za klasu Car
            {
                ProductionYear = year;
            ...
            public bool Equals(Car car) // provjera jednakosti dva objekta klase Car
            {
            ...
            public override string ToString() { return "Model: " + Model + ", color: " + Color; }  // ispis automobila
        }
        ...
        public class Airplane : Vehicle
        {
            ...
            public Airplane(int year, string model)  // dva konstruktora
            {
                ProductionYear = year;
                _model = model;
                TotalTravelLengthKm = 0;
            ...
            public virtual void Transport(double timeH, double avgSpeedKmH, double transportPrice, int transportedEntityQuantity) // virtualna metoda transport
            {
        ...
        public class PassengerPlane : Airplane
        {
            ...
            private List<int> _passengerRatings = new List<int> {}; // ocjene putnika
        ...
        public class Company<T> // generic klasa Company (kompanija može biti avio ili s nekim drugim prijevoznim sredstvima) 
        {
            private List<T> _companyVehicles = new List<T> { }; // lista vozila kompanije

            public List<T> CompanyVehicles { get => _companyVehicles; }
            
            public Company(params T[] vehicles) // konstruktor koji prima neograničen broj vozila i sprema ih u listu vozila kompanije
            {
            ...
            public void PrintCompanyVehicles()  // ispis vozila kompanije
            {
                Console.WriteLine("Company vehicles:");
                foreach (var vehicle in _companyVehicles)
                {
                    Console.WriteLine(vehicle.ToString()); // potreban nam je ToString override u svakoj klasi koju planiramo ovdje koristiti
                }
            }
        }
